...
export class QueueModel {
  ...
  markWhoCancel(db: knex, queueId: any,fullname: any) {
    return db('q4u_queue')
      .where('queue_id', queueId)
      .update({
  ...
  getWorkingDepartment(db: knex, dateServ: any, departmentId: any) {
    ...
    const sql = db('q4u_queue as q')
      .select('q.service_point_id', 'q.queue_interview', 'q.date_serv as queue_date', 'q.room_id',
        'q.queue_number', 'q.hn', 'q.vn', 'q.queue_id', 'q.date_serv', 'q.time_serv', 'p.title', 'p.first_name as fname', 'p.last_name',
        'p.birthdate', 'pr.priority_name', 'pr.priority_id', 'pr.priority_color', 'q.is_completed','pr.priority_thai',
  ...
  getWorkingDepartmentEr(db: knex, dateServ: any, departmentId: any) {
  ...
  getAllQueueActive(db: knex, dateServ: any ,query = '') {
...
//สร้างใหม่ 11/11/2566  เอาไว้คนหาคิวทั้งหมด ที่ยังไม่ได้ส่งต่อ
  getAllQueuePending(db: knex, dateServ: any ,query = '') {
    query = query.replace(/'/g,'');
    const _query = `%${query}%`;
    const sql = db('q4u_queue as q')
      .select(
        'q.queue_number', 'q.hn', 'q.vn', 'q.queue_id', 'q.room_id', 'r.room_name', 'r.room_number',
        'q.date_serv', 'q.time_serv', 'p.title', 'p.first_name', 'p.last_name',
        'p.birthdate', 'pr.priority_name', 'pr.priority_id', 'pr.priority_color', 'sp.service_point_name','q.who_transfer_cancel')
      .innerJoin('q4u_person as p', 'p.hn', 'q.hn')
      .innerJoin('q4u_priorities as pr', 'pr.priority_id', 'q.priority_id')
      .innerJoin('q4u_service_points as sp', 'sp.service_point_id', 'q.service_point_id')
      .leftJoin('q4u_service_rooms as r', 'r.room_id', 'q.room_id')
...
sql.where((w) => {
  ...
  getNextQueueDepartmentER(db: knex, departmentId: number, dateServ, limit: number = 5) {
    const sql = db('q4u_queue as q')
      .join('q4u_service_points as sp', 'sp.service_point_id', 'q.service_point_id')
      ...
      //.orderByRaw(`FIELD(q.priority_id, "11", "3", "2", "10", "1", "6", "7", "8", "9")`)
      .orderBy('q.queue_running', 'ASC')
      .limit(limit);
    return sql;
