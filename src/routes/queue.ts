...
//ค้นหาจาก register
  fastify.post('/all-queue/active', { preHandler: [fastify.authenticate] }, async (req: fastify.Request, reply: fastify.Reply) => {
   
    const query = req.body.query || '';
    try {
      const dateServ: any = moment().format('YYYY-MM-DD');
      const rs: any = await queueModel.getAllQueueActive(db, dateServ,query);
      reply.status(HttpStatus.OK).send({ statusCode: HttpStatus.OK, results: rs });
    } catch (error) {
      fastify.log.error(error);
      reply.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ statusCode: HttpStatus.INTERNAL_SERVER_ERROR, message: HttpStatus.getStatusText(HttpStatus.INTERNAL_SERVER_ERROR) });
...
//สร้างใหม่ 11/11/2566  เอาไว้คนหาคิวทั้งหมด ที่ยังไม่ได้ส่งต่อ
  fastify.post('/all-queue/find-pending', { preHandler: [fastify.authenticate] }, async (req: fastify.Request, reply: fastify.Reply) => {
   
    const query = req.body.query || '';
    try {
      const dateServ: any = moment().format('YYYY-MM-DD');
      const rs: any = await queueModel.getAllQueuePending(db, dateServ,query);
      reply.status(HttpStatus.OK).send({ statusCode: HttpStatus.OK, results: rs });
    } catch (error) {
      fastify.log.error(error);
      reply.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ statusCode: HttpStatus.INTERNAL_SERVER_ERROR, message: HttpStatus.getStatusText(HttpStatus.INTERNAL_SERVER_ERROR) });
...
//สร้าง 24/01/2566 หารายละเอียดการนัด
  fastify.post('/find-patient-appoint', { preHandler: [fastify.authenticate] }, async (req: fastify.Request, reply: fastify.Reply) => {
    const hn = req.body.hn;
  //  const query = req.body.query || '';
    try {
      // const dateServ: any = moment().format('YYYY-MM-DD');
    
      const rs: any = await hisModel.getAppointInfo(dbHIS,hn);
      reply.status(HttpStatus.OK).send({ statusCode: HttpStatus.OK, results: rs });
    } catch (error) {
      fastify.log.error(error);
  ...
  fastify.post('/pendingcancel', { preHandler: [fastify.authenticate] }, async (req: fastify.Request, reply: fastify.Reply) => {
    ...
    try {
      ...
      if (rsInfo) {
        ...
        if (useOldQueue === 'Y') {
          ...
          fastify.mqttClient.publish(topicDepartment2, '{"message":"update_visit"}', { qos: 0, retain: false });
          if (fullname.length) { 
          await queueModel.markWhoCancel(db, newQueueId[0], fullname); 
          }
          reply.status(HttpStatus.OK).send({ statusCode: HttpStatus.OK, queueNumber: strQueueNumber, queueId: newQueueId[0] });
  ...
  fastify.get('/next-queue/department-er', { preHandler: [fastify.authenticate] }, async (req: fastify.Request, reply: fastify.Reply) => {

    const departmentId = +req.query.departmentId;
    const dateServ: any = moment().format('YYYY-MM-DD');
    const limit = +req.query.limit || 15;
    try {
      const rs: any = await queueModel.getNextQueueDepartmentER(db, departmentId, dateServ, limit);
      reply.status(HttpStatus.OK).send({ statusCode: HttpStatus.OK, results: rs });
    } catch (error) {
      fastify.log.error(error);
      reply.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ statusCode: HttpStatus.INTERNAL_SERVER_ERROR, message: HttpStatus.getStatusText(HttpStatus.INTERNAL_SERVER_ERROR) });
